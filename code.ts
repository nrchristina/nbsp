...
function replaceSpaces(text: string): { result: string; collapsedSpaces: number; nbspCount: number } {
  let result = text;
...
async function processText(scope: 'selection' | 'page' | 'file'): Promise<{ success: boolean; collapsedSpaces: number; nbspCount: number; error?: string }> {
